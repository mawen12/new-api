...
func AnonymousRequestBodyLimit() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if maxBytes <= 0 || c.Request.Body == nil { // 不限制
			c.Next()
			return
		}
		...
		if err != nil {
			if common.IsRequestBodyTooLargeError(err) { // 请求体超过限制，返回 413 错误
				c.AbortWithStatus(http.StatusRequestEntityTooLarge)
				return
			}
			c.AbortWithStatus(http.StatusBadRequest) // 请求体格式错误，返回 400
			return
		}
...
// readAnonymousRequestBody 读取请求体
